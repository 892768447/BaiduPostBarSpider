...
from BaiduPostBarSpider.items import ForumListItem, ForumInfosItem, \
    LzlCommentItem
...
class BaiduPostBarSpider(Spider):
    ...
    #data_field字段
    DataFieldXpath = '@data-field'
    # 提取页面帖子信息
    ForumListXpath = '//li[normalize-space(@class)="j_thread_list clearfix"]'
    #提取页面帖子标题信息
    ForumListTitleXpath = '*//a[normalize-space(@class)="j_th_tit"]/text()'
    # 提取页面帖子链接
    ForumUrlXpath = '*//a[normalize-space(@class)="j_th_tit"]/@href'
    # 提取帖子标题
    ...
    # 提取楼中楼回复
    ForumContentsLzl = '//span[normalize-space(@class)="lzl_content_main"]/text()'
    # 楼中楼评论时间
    ForumContentsTime = '//span[normalize-space(@class)="lzl_time"]/text()'
    # 楼中楼评论用户昵称是pid
    ForumContentsData = '//li[contains(@class,"lzl_single_post j_lzl_s_p")]/@data-field'
...
#         crawler.settings["Session"] = dbInit()
    ...
    def parse_forum_list(self, response):
        ...
        for uri in response.xpath(self.ForumUrlXpath).extract():
            ...
            yield Request(self.ForumUrl.format(uri=uri),
                          ...
                          meta={"post_id": uri.split("/")[2], "next_page":1},
    ...
    def parse_forum(self, response):
        ...
        post_title = response.meta.get("post_title", response.xpath(self.ForumTitleXpath).extract_first()),  # 标题
        ...
        item = ForumInfosItem(
            ...
            post_content=forum_infos.xpath(self.ForumContentsXpath).extract(),  # 评论
        ...
        try:
            ...
            if _content:
                ...
                for pn in range(1, lzl_page_num):
                    yield Request(
                        self.LzlUrl.format(tid=post_id, pid=post_pid, pn=pn, t=time()),
                              meta={"post_id": post_id, "post_pid":post_pid, "post_title":post_title},
